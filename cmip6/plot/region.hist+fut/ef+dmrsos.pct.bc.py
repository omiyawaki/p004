...
def proc_data(sfo, syr, igpi):
    ...
    vn1, mvn1 =load_data(md,sfo,varn1,igpi,syr)
    print('Loading %s...'%varn2)
    vn2, mvn2 =load_data(md,sfo,varn2,igpi,syr)
    print('Loading tas...')
    tas, mtas =load_data(md,sfo,'tas',igpi,syr)
...
def plot(md, relb):
    ...
    # load bc
    bch = get_bc(md, fo1, yr1, igpi)
    bcf = get_bc(md, fo2, yr2, igpi)

    print('Plotting...')
    tname=r'%s' % (md)
    oname1=f'{odir}/{varn}_{yr1}+{yr2}.{se}.{relb}.{mo}.pct.{varnbc}'
    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    clf=ax.scatter(vn2h,vn1h,s=0.5,c=btash, label=fo1, cmap='Blues')
    ax.scatter(vn2f,vn1f,s=0.5,c=btasf, label=fo2, cmap='Oranges')
    ax.plot(bch[0]+mvn2h.data, bch[1], color='tab:blue')
    ax.plot(bcf[0]+mvn2f.data, bcf[1], color='tab:orange')
