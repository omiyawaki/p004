...
varn1b='hfls'
varn2='mrsos'
varn=f'{varn1a}_div_{varn1b}+{varn2}'
se='sc'
mo=7
...
def plot(relb,fo=fo1,yr=yr1):
    ...
    odir= '/project/amp/miyawaki/plots/p004/cmip6/%s/%s/%s/%s/%s/bc/%s' % (se,fo,md,varn,'regions', relb)
    if not os.path.exists(odir):
        os.makedirs(odir)

    print('Loading %s...'%varn1a)
    vn1a=load_data(md,fo,varn1a,igpi,yr)
    print('Loading %s...'%varn1b)
    vn1b=load_data(md,fo,varn1b,igpi,yr)
    vn1 = vn1a / vn1b
    ...
    print('Plotting...')
    tname=r'%s' % (md)
    oname1=f'{odir}/{varn}_{yr}.{se}.{relb}.{mo}.bc'
    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    if fo == 'historical':
        clf=ax.scatter(vn2,vn1,s=0.5,c='tab:blue')
    elif fo == 'ssp370':
        clf=ax.scatter(vn2,vn1,s=0.5,c='tab:orange')
    ...
    ax.set_ylabel(f'$\\frac{ {varnlb(varn1a)} }{ {varnlb(varn1b)} }$ ({unitlb(varn1a)} / {unitlb(varn1b)})')
