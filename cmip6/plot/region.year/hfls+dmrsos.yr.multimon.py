...
def plot(relb,fo=fo1,yr=yr1):
    ...
    odir= '/project/amp/miyawaki/plots/p004/cmip6/%s/%s/%s/%s/%s/yr/%s' % (se,fo,md,varn,'regions', relb)
    if not os.path.exists(odir):
        os.makedirs(odir)

    print('Plotting...')
    tname=r'%s' % (md)
    oname1=f'{odir}/{varn}_{yr}.{se}.{relb}.{lmo}.yr'
    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    for mo in tqdm(lmo):
        svn1, svn2 = proc(mo, fo, yr, igpi)
        clf=ax.scatter(svn2,svn1,s=0.5,c=svn1['time.year'],cmap='viridis')
